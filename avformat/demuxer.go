...
func (s *BaseDemuxer) OnVideoPacket(bufferIndex int, id utils.AVCodecID, data []byte, key bool, dts, pts int64, packType PacketType) {
	// log.Print("avformat/demuxer.go -> OnVideoPacket ")
	var ok bool
	defer func() {
		if !ok {
			s.DataPipeline.DiscardBackPacket(bufferIndex)
	...
	// If the track does not exist and is packaged in AnnexB, extract sps and pps from NALU to encapsulate AVStream
	track := s.findTrackByBufferIndex(bufferIndex)
...
// Callback AVPacket. If detection is not completed, save it to Packets, otherwise call back
// Ensure the callback order is OnNewTrack...->OnTrackComplete->OnPacket...
func (s *BaseDemuxer) processBufferedPacket(packet *AVPacket) {
	...
	// Calculate the duration of the previous AVPacket
	prevPacket := packets.Get(prevPacketIndex)
	prevPacket.Duration = packet.Dts - prevPacket.Dts

	// If the detection is completed, the callback is processed
	if s.Completed {
